...
impl ProtocolValidator {
    ...
    #[cfg(not(target_arch = "wasm32"))]
    fn compile_schema_from_str(schema_content: &str) -> Result<JSONSchema, ProtocolError> {
        ...
        #[cfg(not(target_arch = "wasm32"))]
        {
            ...
            let schema_content = Self::load_schema_from_local()
                ...
                .or_else(|| {
                    ...
                    if let Ok(root) = std::env::var("AI_PROTOCOL_DIR")
            ...
            let schema = Self::compile_schema_from_str(&schema_content)?;
  ...
  "additionalProperties": true
    ...
    pub fn validate(&self, manifest: &ProtocolManifest) -> Result<(), ProtocolError> {
        #[cfg(target_arch = "wasm32")]
        {
            return Self::validate_basic(manifest);
        }
