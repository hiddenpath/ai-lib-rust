...
impl ProtocolManifest {
    ...
    pub fn compile_request(
        ...
        // Map response_format if present (structured output support)
        if let Some(ref json_mode_config) = request.response_format {
            let openai_format = json_mode_config.to_openai_format();
            // Check if provider accepts response_format mapping
            if let Some(mapped) = self.parameter_mappings.get("response_format") {
                PathMapper::set_path(&mut provider_request, mapped, openai_format).map_err(
                    |e| {
                        ProtocolError::ValidationError(format!(
                            "Failed to set response_format: {}",
                            e
                        ))
                    },
                )?;
            } else {
                // If no mapping is defined, we can still try to set it directly if provider accepts OpenAI format
                // This is a simple heuristic that works for OpenAI-compatible providers
                provider_request["response_format"] = openai_format["response_format"].clone();
            }
        }
...
>>>>>>> 725d9ae (feat: add structured output support with JSON schema validation)
